...
fun PhoneFieldRow(
    phone: PhoneDto,
    error: String?,
    onUpdate: (PhoneDto) -> Unit,
    onRemove: () -> Unit
) {
    var expanded by remember { mutableStateOf(false) }
    val typeLabel = phoneTypeLabel(type = phone.type, label = phone.label)
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            OutlinedTextField(
                value = phone.value,
                onValueChange = { onUpdate(phone.copy(value = it)) },
        ...
        ExposedDropdownMenuBox(
            ...
            ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                ContactTypeLabels.phoneTypes.forEach { (typeStr, label) ->
                    DropdownMenuItem(
                        text = { Text(label) },
                        onClick = {
                            onUpdate(phone.copy(type = typeStr))
                            expanded = false
                        }
                    )
                }
        ...
        IconButton(onClick = onRemove, modifier = Modifier.size(48.dp)) {
            Icon(Icons.Default.Remove, contentDescription = "Remove phone")
        }
    }
}
